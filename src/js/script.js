...
const keyEvent = (event, btn, codeEvent) => {
  ...
  if (codeEvent === 'ArrowUp') {
    ...
    if (SymbolBeforeCursor.length === 1
      || SymbolBeforeCursor[SymbolBeforeCursor.length - 1].length >= 55) {
      cursor -= 55;
    } else if (SymbolBeforeCursor[SymbolBeforeCursor.length - 1].length
      <= SymbolBeforeCursor[SymbolBeforeCursor.length - 2].length % 55) {
  ...
  if (codeEvent === 'ArrowDown') {
    ...
    if (symbolAfterCursor.length === 1 || symbolAfterCursor[0].length >= 55) {
     ...
     + symbolAfterCursor[0].length) > 57)) {
      cursor += symbolAfterCursor[0].length;
    } else {
      cursor += (symbolBeforeCursor[symbolBeforeCursor.length - 1].length % 57)
      + symbolAfterCursor[0].length + 1;
    }
    textField.setSelectionRange(cursor, cursor);
  }
